...
{
    [TestClass]
    public sealed class ItemSerialEncoderTests
    {
        [TestMethod]
        public void TestEncoder()
        {
            ...
            var base85Decoder = new Base85Decoder();
            var decoder = new ItemSerialDecoder();
            var encoder = new ItemSerialEncoder();

            foreach (var serial in samples)
            {
                var bitstream = base85Decoder.DecodeToBitStream(serial);
                var results = decoder.DecodeBitstream(bitstream, debug: false);

                var partStr = decoder.FormatResults(results);

                var encodedSerial = encoder.EncodeToSerial(partStr);

                Assert.AreEqual(serial, encodedSerial, true);
            }
        }
    }
}
